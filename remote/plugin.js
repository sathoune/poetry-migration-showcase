...
const a = function () {
    ...
    function init(reveal) {
      ...
      function init() {
        ...
        socket.on("connect_error", function (err) {
          console.warn(
            "Remote: Could not connect to socket.io-remote server",
            err
          );
        });
        socket.on("reconnect_error", function (err) {
          console.warn(
            "Remote: Could not reconnect to socket.io-remote server",
            err
          );
        });
        socket.on("connect_timeout", function () {
          console.warn(
            "Remote: Could not connect to socket.io-remote server (timeout)"
          );
        });
        socket.on("reconnect_failed", function (err) {
          console.warn(
            "Remote: Could not reconnect to socket.io-remote server - this was the last try, giving up",
            err
          );
        });
      ...
      function msgInit(data) {
        if (pluginConfig.remote) {
          reveal.addKeyBinding(
            { keyCode: 82, key: "R", description: "Show remote control url" },
            function () {
        ...
        if (pluginConfig.multiplex) {
          reveal.addKeyBinding(
            { keyCode: 65, key: "A", description: "Show share url" },
            function () {
      ...
      function sendRemoteState() {
        socket.emit("state_changed", {
          ...
          autoslide:
            ...
            config.autoSlide > 0 &&
            (typeof config.autoSlideStoppable !== "boolean" ||
      ...
      function msgCommand(data) {
        ...
        if (listeners.hasOwnProperty(cmd)) {
          ...
          console.log(
            "Remote: No listener registered for",
            cmd,
  ...
  if (
    ...
    Reveal.hasOwnProperty("VERSION") &&
    Reveal.VERSION.startsWith("3.")
  ) {
