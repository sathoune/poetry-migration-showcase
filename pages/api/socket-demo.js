...
const index = async (res, path) => {
  fs.promises
    ...
    .then((files) => {
      ...
      res.send(
        "<!DOCTYPE html><html lang='en'><head><title>Directory Listing</title></head><ul>" +
          list +
          "</ul>"
      );
    })
...
const parseArgs = () => {
  const optionList = [
    ...
    },
    {
      ...
      description:
        "Enable ssl mode, requires the path to a pfx file as argument (default: none, env: PRESENTATION_PFX_FILE)",
    },
    {
      name: "basepath",
      alias: "b",
      ...
      description:
        "URL basepath where the application can be found (useful for reverse proxys, default: /, env: PRESENTATION_BASE_PATH)",
    },
    {
      ...
      typeLabel: "{underline path}",
      defaultValue:
        process.env.PRESENTATION_PRESENTATION_PATH ||
        __dirname + "/presentations/",
      description:
        "URL basepath where the application presentations can be found (default: ./presentations/, env: PRESENTATION_PRESENTATION_PATH)",
    },
    {
      name: "hashsecret",
      alias: "a",
      ...
      description:
        "A secret which is used to resume a session after the presentation is reloaded (default: a random value, env: PRESENTATION_PRESENTATION_PATH)",
    },
    {
      name: "help",
      alias: "h",
...
createServer(args, app).then((server) => {
  ...
  const io = socketIo(server, {
    path: args.basepath + "socket.io",
    cookie: false,
  });
  io.sockets.on("connection", (socket) =>
    initConnection(socket, prefix, args.hashsecret, args.ssl !== null)
  );
